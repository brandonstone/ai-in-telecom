...
Port scanning in telecom is particularly significant because signaling protocols (SS7, Diameter, SIP) expose services on discoverable ports. A DDoS against a telecom node disrupts service for entire subscriber segments, not just one target. Slow exfiltration has no signature because the pattern is novel by definition — the value of Zeek is detecting it without one.
...
|---|---|
| This lab | Synthetic traffic, single subnet, low thresholds — both systems alert frequently |
...
Results are deterministic — the same traffic generator script produces the same packet sequence every run. To reset and rerun from scratch:
...
| Issue | Fix |
|---|---|
| Snort AMD64-only, broken under Apple Silicon | Switched to Suricata — native ARM64, same rule syntax |
...
| Suricata reputation preprocessor error | Commented out — not compiled into the image |

---

## Notes

- Suricata is used in place of Snort throughout. Both are signature-based IDS tools using identical rule syntax. The substitution was made due to ARM64 platform constraints (Apple Silicon) — the `linton/docker-snort` image is AMD64-only and its rule engine fails silently under QEMU emulation.
